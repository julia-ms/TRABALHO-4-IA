...
def evaluate_mask(state, player:str) -> float:
    ...
    for index_x, linha in enumerate(EVAL_TEMPLATE):
        for index_y, score in enumerate(linha):
            if state.board.tiles[index_x][index_y] == "B":
                score_B += score
            elif state.board.tiles[index_x][index_y] == "W":
                score_W += score
